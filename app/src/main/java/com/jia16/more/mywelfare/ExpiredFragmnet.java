 ...
 * 更多 ------我的福利 的已过期界面
 */
public class ExpiredFragmnet extends BaseListFragment<Ticket> {

    private UserInfo userInfo;
    private int userId;
    private String amount;//未使用的代金券的总金额
    ...
    private void postUnuserdDatas1() {
        String url = "";
        if (list.size() == 0) {
            url = "/api/users/" + userId + "/simple-vouchers?status=EXPIRED&page=1&pageLimit=10&queryFlag=3";
        } else {
            int page = Integer.valueOf(list.size() / 10).intValue() + 1;
            url = "/api/users/" + userId + "/simple-vouchers?status=EXPIRED&page=" + page + "&pageLimit=10&queryFlag=3";
        }
    ...
    private void postUnuserdDatas2() {
        String url = "";
        if (list.size() == 0) {
            url = "/api/users/" + userId + "/simple-vouchers?status=EXPIRED&page=1&pageLimit=10&queryFlag=1";
        } else {
            int page = Integer.valueOf(list.size() / 10).intValue() + 1;
            url = "/api/users/" + userId + "/simple-vouchers?status=EXPIRED&page=" + page + "&pageLimit=10&queryFlag=1";
        }
        url = UrlHelper.getUrl(url);
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.GET, url, null,
                new Response.Listener<JSONObject>() {
    ...
    private void postUnuserdAmount() {
        ...
        String url = "/api/users/" + userId + "/voucher-total-amount?status=EXPIRED";
        url = UrlHelper.getUrl(url);
    ...
    public void addHeadView() {
        ...
        mTvWelfareDesc.setText("已过期代金券总额 ");
