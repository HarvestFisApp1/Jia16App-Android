 ...
 * 更多 ------我的福利 的未使用界面
 */
public class UnUserdFragmnet extends BaseListFragment<Ticket> {
    ...
    private void postUnuserdDatas1() {
        String url = "";
        if (list.size() == 0) {
            url = "/api/users/" + userId + "/simple-vouchers?status=UNUSED&page=1&pageLimit=10&queryFlag=3";
        } else {
            int page = Integer.valueOf(list.size() / 10).intValue() + 1;
            url = "/api/users/" + userId + "/simple-vouchers?status=UNUSED&page=" + page + "&pageLimit=10&queryFlag=3";
        }
    ...
    private void postUnuserdDatas2() {
        String url = "";
        if (list.size() == 0) {
            url = "/api/users/" + userId + "/simple-vouchers?status=UNUSED&page=1&pageLimit=10&queryFlag=1";
        } else {
            int page = Integer.valueOf(list.size() / 10).intValue() + 1;
            url = "/api/users/" + userId + "/simple-vouchers?status=UNUSED&page=" + page + "&pageLimit=10&queryFlag=1";
        }
        url = UrlHelper.getUrl(url);
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.GET, url, null,
                new Response.Listener<JSONObject>() {
