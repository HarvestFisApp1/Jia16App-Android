 ...
 * 我要投资  界面--投资期限
 */
public class InvestDateFragmnet extends BaseListFragment<InvestConstant> {
    ...
    private void postUnuserdDatas1() {
        ...
        if (list.size() == 0) {
            if(BaseApplication.getInstance().isLogined()){
                url = "/api/subjects/app-fix-income?page=1&pageLimit=5&timestamp="+currentTimeInLong+"&catalog=JIASHI_V1&catalog=JIASHI_V2&catalog=JIASHI_V3&catalog=JIASHIV4&userId="+userId+"&orderFlag=3";
            }else {
                url = "/api/subjects/app-fix-income?page=1&pageLimit=5&timestamp="+currentTimeInLong+"&catalog=JIASHI_V1&catalog=JIASHI_V2&catalog=JIASHI_V3&catalog=JIASHIV4&orderFlag=3";
            }

        } else {
            int page = Integer.valueOf(list.size() / 6).intValue() + 1;
            if(BaseApplication.getInstance().isLogined()){
                url = "/api/subjects/app-fix-income?page="+page+"&pageLimit=5&timestamp="+currentTimeInLong+"&catalog=JIASHI_V1&catalog=JIASHI_V2&catalog=JIASHI_V3&catalog=JIASHIV4&userId="+userId+"&orderFlag=3";
            }else {
                url = "/api/subjects/app-fix-income?page="+page+"&pageLimit=5&timestamp="+currentTimeInLong+"&catalog=JIASHI_V1&catalog=JIASHI_V2&catalog=JIASHI_V3&catalog=JIASHIV4&userId&orderFlag=3";
            }
    ...
    private void postUnuserdDatas2() {
        String url = "";
        if (list.size() == 0) {
            url = "/api/subjects/app-fix-income?page=1&pageLimit=5&timestamp="+currentTimeInLong+"&catalog=JIASHI_V8&catalog=JIASHI_V9&catalog=JIASHI_V13&status=FUNDING&status=FUNDED&status=PORTION_FUNDED&orderFlag=3";
        } else {
            int page = Integer.valueOf(list.size() / 6).intValue() + 1;
            url = "/api/subjects/app-fix-income?page="+page+"&pageLimit=5&timestamp="+currentTimeInLong+"&catalog=JIASHI_V8&catalog=JIASHI_V9&catalog=JIASHI_V13&status=FUNDING&status=FUNDED&status=PORTION_FUNDED&orderFlag=3";
        }
        url = UrlHelper.getUrl(url);
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.GET, url, null,
                new Response.Listener<JSONObject>() {
